...
class CapacityRiskCalculator:
    ...
    def _calculate_metrics(self) -> dict:
        ...
        # Reset stop_flag for the start of a new run (first shot is normal production)
        ...
        total_capacity_loss_sec = downtime_sec + (capacity_loss_slow_parts * avg_approved_ct) 
        ...
        # REMOVED: df.loc[is_new_run, 'shot_type'] = 'Run Break (Excluded)' 
        # Rationale: The first shot of a new run is a normal production shot and should be classified as such.
...
def calculate_run_summaries(df_period, config):
    ...
    for r_id, df_run in df_period.groupby('run_id'):
        ...
        summary_list.append({
            ...
            'mttr_min': res['mttr_min'], # Added for correlation
            'stability_index': res['stability_index'] # Added for correlation
        })
    # Resetting the index to create a sequential run label (Run 001, Run 002) for display purposes
    # regardless of the original run_id which might be high due to filtering elsewhere.
...
def generate_capacity_insights(res, benchmark_mode):
    ...
    if total_loss_absolute > 0:
        ...
        # Contextualize language based on status
...
def prepare_and_generate_capacity_excel(df_view, config):
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        if not run_data.empty:
            cols = ['start_time', 'end_time', 'optimal_output_parts', 'actual_output_parts', 
                    'capacity_loss_downtime_parts', 'capacity_loss_slow_parts', 'capacity_gain_fast_parts']
...
def generate_mttr_mtbf_analysis(analysis_df):
    ...
    # Check for sufficient data points (minimum 2 runs with data)
    ...
    # Rename for consistency with logic
    # Use 'display_run_id' if available to match the breakdown table, otherwise fallback to 'run_id'
...
def create_time_breakdown_donut(total_sec, prod_sec, down_sec):
    """Creates a donut chart for Total Run Time Breakdown."""
    # Colors
    c_prod = PASTEL_COLORS['green']
    c_down = PASTEL_COLORS['red']
    
    prod_pct = (prod_sec / total_sec * 100) if total_sec > 0 else 0
    down_pct = (down_sec / total_sec * 100) if total_sec > 0 else 0
    
    # Text - Increased font size and improved layout to prevent overlap
...
def create_modern_gauge(value, title):
    ...
    fig = go.Figure(data=[go.Pie(
        ...
        sort=False,
        direction='clockwise',
        rotation=-90, # Starts at 9 o'clock
        textinfo='none',
    ...
    fig.add_annotation(
        text=f"{value:.1f}%",
        x=0.5, y=0.15, # Lowered y-pos for half-circle center to avoid overlap
...
def create_stability_driver_bar(mtbf, mttr, stability_index):
    """Creates a stacked horizontal bar chart for MTBF vs MTTR with Legend-based labels."""
    total = mtbf + mttr
    ...
    # Calculate downtime percentage from stability index for the footnote
    downtime_pct = 100 - stability_index
    
    # Labels for Legend (Percentage corrected to .1f)
    ...
    # Generate Smart Footnote (Correlation Proxy)
    ...
    fig.update_layout(
        ...
        # Generous margins: b=120 to fit legend and spaced footnote
        ...
        paper_bgcolor='rgba(0,0,0,0)', 
        plot_bgcolor='rgba(0,0,0,0)',
        # Footnote: Positioned lower for double spacing
        annotations=[
...
def plot_prediction_chart(pred_data, demand_target_total=None):
    """
    Renders the 'Burn-up' chart for future projection.
    """
    if not pred_data: return go.Figure()

    fig = go.Figure()

    # 1. Historical Actuals (Solid Blue)
    ...
    # 2. Projected Average (Dashed Blue)
    fig.add_trace(go.Scatter(
    ...
    # 3. Projected Peak (Dashed Green) - Optional Context
    fig.add_trace(go.Scatter(
        x=pred_data['future_dates'], 
    ...
    # 4. Required Rate (Red/Orange Dashed) - If Target Set
...
def plot_shot_analysis(df_shots, zoom_y=None):
    ...
    # Add purple dashed lines for new run starts
    if 'run_id' in df_shots.columns:
        # Group by run_id and find the minimum shot_time for each run
        run_starts = df_shots.groupby('run_id')['shot_time'].min().sort_values()
        
        # Iterate through the start times, skipping the very first one if it's the start of the chart
        for i, start_time in enumerate(run_starts):
            if i > 0: # Skip the first run's start to avoid a line at the very beginning
