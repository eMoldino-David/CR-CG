...
def load_po_data(files):
    ...
    for file in files:
        try:
            ...
            mapping = {"PO_NUMBER": "po_number", "PROJECT": "project", "PART_ID": "part_id", "TOTAL_QTY": "total_qty", "START_DATE": "start_date", "DUE_DATE": "due_date"}
...
class CapacityRiskCalculator:
    ...
    def _calculate(self):
        ...
        optimal_output = (total_runtime_sec / df['approved_ct'].mean()) * df['working_cavities'].mean()
...
def create_modern_gauge(value, title):
    ...
    fig.add_annotation(text=f"{value:.1f}%", x=0.5, y=0.2, font=dict(size=36, color='white', weight='bold'), showarrow=False)
    fig.update_layout(title=dict(text=title, x=0.5, font=dict(size=16)), margin=dict(l=20, r=20, t=40, b=0), height=200, showlegend=False, paper_bgcolor='rgba(0,0,0,0)')
    return fig
...
def create_time_donut(total_sec, prod_sec, down_sec):
    ...
    fig.update_layout(title="Run Duration Breakdown", showlegend=True, legend=dict(orientation="h", y=-0.1), height=280, margin=dict(t=40, b=40, l=20, r=20), paper_bgcolor='rgba(0,0,0,0)')
    return fig

def create_hybrid_chart(df_agg, cal_config):
    fig = go.Figure()
    fig.add_trace(go.Bar(x=df_agg['Period'], y=df_agg['Actual Output'], name='Actual Output', marker_color=PASTEL_COLORS['blue'], yaxis='y1'))
    ...
    if cal_config:
        ...
        df_agg['Stress'] = (df_agg['Run Time Sec'] / 3600 / planned * 100).clip(upper=200)
    ...
    fig.update_layout(
        ...
        legend=dict(orientation="h", y=-0.2), height=450, paper_bgcolor='rgba(0,0,0,0)'
    )
    return fig

def plot_shot_analysis(df_shots):
    fig = go.Figure()
    colors = {'Slow Cycle': PASTEL_COLORS['red'], 'Fast Cycle': PASTEL_COLORS['orange'], 'On Target': PASTEL_COLORS['blue'], 'Downtime (Stop)': '#808080'}
